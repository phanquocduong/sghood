...
class MeterReadingService
{
    ...
    /**
     * Lấy phòng cần nhập chỉ số trong thời gian quy định (26 -> 5)
     */
    public function getRoomsWithMotel()
    {
        try {
            ...
            if ($currentDay >= 26) {
                // Từ ngày 26 trở đi của tháng hiện tại -> nhập chỉ số cho tháng hiện tại
                ...
                // Ngoài thời gian cho phép (26-27) -> không có phòng nào cần nhập
                Log::info('Outside meter reading period', [
                    'current_date' => $today->toDateString(),
                    'current_day' => $currentDay,
                    'message' => 'Not in allowed period (26th-5th next month)'
                ]);
                return collect();
            }
    ...
    public function getDisplayPeriodInfo()
    {
        ...
        $isInAllowedPeriod = ($currentDay >= 26) || ($currentDay <= 5);

        if ($currentDay >= 26) {
            // Từ ngày 26 trở đi -> nhập chỉ số cho tháng hiện tại
            $displayMonth = $today->month;
            $displayYear = $today->year;
            $periodDescription = "Nhập chỉ số tháng {$displayMonth}/{$displayYear}";

            $startDate = $today->copy()->day(26)->startOfDay();
            ...
            $startDate = $previousMonth->copy()->day(26)->startOfDay();
            ...
            $displayMonth = $today->month;
            $displayYear = $today->year;
            $periodDescription = "Ngoài thời gian nhập chỉ số (chỉ từ 26 - 5 tháng sau)";

            // Tính toán kỳ tiếp theo
            $nextPeriodStart = $today->copy()->day(26);
        ...
        try {
            // Kiểm tra xem ngày 26 có tồn tại trong tháng không
            $testDate = $today->copy();
            if ($testDate->day > 26) {
                $testDate->day(26); // Có thể throw exception nếu tháng không có ngày 26
            }
        } catch (\Exception $e) {
