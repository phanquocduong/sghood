...
@section('title', 'Thùng rác khu vực')
...
<div class="container-fluid py-5 px-4">
    <div class="card shadow-lg border-0" style="border-radius: 15px; background: #fff;">
        <div class="card-header bg-gradient text-white d-flex justify-content-between align-items-center" style="background: linear-gradient(90deg, #007bff, #00c6ff); border-top-left-radius: 15px; border-top-right-radius: 15px;">
            <h6 class="mb-0 fw-bold">{{ __('Thùng rác khu vực') }}</h6>
            <div>
                    ...
                    <tbody>
                        @forelse ($districts as $index => $district)
                            <tr class="table-row">
                                ...
                                <td>{{ $district->name }}</td>
                                <td>{{ $district->deleted_at->format('d/m/Y H:i') }}</td>
                                <td>
                                    <form action="{{ route('districts.restore', $district->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    <form action="{{ route('districts.forceDelete', $district->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center text-muted py-4">Không có khu vực nào trong thùng rác.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
