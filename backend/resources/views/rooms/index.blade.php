...
@section('content')
    ...
    <div class="container-fluid py-5 px-4">
        <div class="card shadow-lg border-0" style="border-radius: 15px; background: #fff;">
            <div class="card-header bg-gradient text-white d-flex flex-row justify-content-between align-items-center flex-wrap"
                style="background: linear-gradient(90deg, #007bff, #00c6ff); border-top-left-radius: 15px; border-top-right-radius: 15px;">

                <h6 class="mb-0 fw-bold d-flex align-items-center">
                    {{ __('Danh sách phòng trọ') }}
                    <span class="badge bg-light text-primary ms-2">{{ $motel->name ?? 'ID: ' . $motelId }}</span>
                </h6>

                <div class="d-flex flex-row mt-2 mt-sm-0">
                    <a href="{{ route('rooms.create', ['motel_id' => $motelId]) }}" class="btn btn-primary me-2 shadow-sm"
                        style="transition: all 0.3s;">
                        <i class="fas fa-plus me-1"></i> {{ __('Thêm phòng trọ') }}
                    </a>
                    <a href="{{ route('rooms.trash', ['motel_id' => $motelId]) }}" class="btn btn-danger shadow-sm"
                        style="transition: all 0.3s;">
                        ...
                        <div class="col-md-6">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control shadow-sm" name="query"
                                    placeholder="Tìm kiếm phòng trọ..." value="{{ $querySearch }}">
                            </div>
                        </div>
                        <div class="col-md-3">
                        ...
                        <tbody>
                            @forelse ($rooms as $index => $room)
                                <tr class="table-row">
                                    ...
                                    <td>
                                        @if ($room->main_image)
                                            <img src="{{ asset($room->main_image->image_url) }}" alt="{{ $room->name }}"
                                    ...
                                    <td>
                                        <div class="d-flex flex-column">
                                            ...
                                            <span class="text-muted">{{ number_format($room->price, 0, ',', '.') }}
                                                đ</span>
                                        </div>
                                    </td>
                                    <td>
