...
@section('content')
    <div class="container-fluid py-5 px-4">
        ...
        <div class="card shadow-lg border-0 rounded-4">
            ...
            <div class="card-body p-4">
                ...
                <div class="mb-4">
                    <form action="{{ route('bookings.index') }}" method="GET" class="row g-3">
                        ...
                        <div class="col-md-1">
                            <a href="{{ route('bookings.index') }}" class="btn btn-outline-secondary shadow-sm w-100"
                                title="Làm mới">
                        ...
                        <tbody>
                            @forelse ($booking as $bookingItem)
                                <tr class="table-row">
                                    ...
                                    <td>
                                        <span class="room-name-clickable" data-room-id="{{ $bookingItem->room->id ?? '' }}"
                                    ...
                                    <td class="text-center">
                                        @if (in_array($bookingItem->status, ['Chờ xác nhận']))
                                            <form action="{{ route('bookings.updateStatus', $bookingItem->id) }}"
                                                method="POST" class="d-inline status-form"
                                                data-booking-id="{{ $bookingItem->id }}">
                                                @csrf
                                                @method('PATCH')
            ...
            <div class="modal fade" id="rejectNoteModal" tabindex="-1" aria-labelledby="rejectNoteModalLabel"
                aria-hidden="true">
