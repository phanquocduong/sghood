        ...
        <div class="card shadow-lg border-0" style="border-radius: 15px; background: #fff;">
            <div class="card-header bg-gradient text-white d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center"
                ...
                <div class="d-flex flex-row w-100 w-sm-auto justify-content-between justify-content-sm-end">
                    <a href="{{ route('amenities.create') }}" class="btn btn-primary me-2 shadow-sm w-50 w-sm-auto"
                        style="transition: all 0.3s;">
                        <i class="fas fa-plus me-1"></i>
                        <span class="d-none d-sm-inline">{{ __('Thêm tiện ích') }}</span>
                    </a>
                    <a href="{{ route('amenities.change-order') }}" class="btn btn-primary me-2 shadow-sm w-50 w-sm-auto"
                        style="transition: all 0.3s;">
                        <i class="fas fa-edit me-1"></i> <span class="d-none d-sm-inline">{{ __('Thay đổi thứ tự') }}</span>
                    </a>
                    <a href="{{ route('amenities.trash') }}" class="btn btn-danger shadow-sm w-50 w-sm-auto"
                        style="transition: all 0.3s;">
                        <i class="fas fa-trash me-1"></i> <span class="d-none d-sm-inline">{{ __('Thùng rác') }}</span>
                    </a>
                </div>
            </div>
                    ...
                    <form action="{{ route('amenities.index') }}" method="GET" class="row g-3">
                        ...
                        <div class="col-md-3">
                            <select class="form-select shadow-sm" name="type" onchange="this.form.submit()">
                                ...
                                <option value="Phòng trọ" {{ ($type ?? '') == 'Phòng trọ' ? 'selected' : '' }}>Phòng trọ
                                </option>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <select class="form-select shadow-sm" name="sortOption" onchange="this.form.submit()">
                                ...
                                <option value="status_asc" {{ $sortOption == 'status_asc' ? 'selected' : '' }}>Hoạt động
                                </option>
                                <option value="status_desc" {{ $sortOption == 'status_desc' ? 'selected' : '' }}>Không hoạt
                                    động</option>
                                <option value="order_asc" {{ $sortOption == 'order_asc' ? 'selected' : '' }}>Thứ tự tăng
                                </option>
                                <option value="order_desc" {{ $sortOption == 'order_desc' ? 'selected' : '' }}>Thứ tự giảm
                                </option>
                                <option value="created_at_asc" {{ $sortOption == 'created_at_asc' ? 'selected' : '' }}>Cũ
                                    nhất</option>
                                <option value="created_at_desc" {{ $sortOption == 'created_at_desc' ? 'selected' : '' }}>
                                    Mới nhất</option>
                            </select>
                        </div>
                    </form>
