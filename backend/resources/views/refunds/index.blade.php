                    ...
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control shadow-sm" name="querySearch"
                                placeholder="Tìm kiếm theo tên người dùng hoặc phòng..."
                                value="{{ $querySearch }}">
                        </div>
                    </div>
                    <div class="col-md-3">
                ...
                <table class="table table-hover table-bordered align-middle shadow-sm rounded-3 overflow-hidden" style="text-align: center">
                    ...
                    <tbody>
                        @forelse ($refunds as $refund)
                            <tr class="table-row">
                                ...
                                <td>
                                    <div class="d-flex align-items-center justify-content-center">
                                        ...
                                        <span class="fw-medium">{{ $refund->checkout->contract->room->name ?? 'N/A' }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center justify-content-center">
                                        <span class="fw-medium">{{ $refund->bank_info['account_holder'] ?? 'N/A' }}</span>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <span class="fw-bold text-success">
                                        {{ number_format($refund->checkout->final_refunded_amount ?? 0, 0, ',', '.') }} VNĐ
                                    </span>
                                </td>
                                <td class="text-center">
                                    @php
                                ...
                                <td class="text-center">
                                    @if($refund->checkout->user_confirmation_status === 'Đồng ý')
                                        <button type="button" class="btn btn-info btn-sm shadow-sm me-2" data-bs-toggle="modal" data-bs-target="#qrModal{{ $refund->id }}" title="Xem chi tiết">
                                            <i class="fas fa-eye me-1"></i>Xem chi tiết
                                        </button>
                                        @if($refund->status === 'Chờ xử lý')
                                            <form action="{{ route('refunds.confirm', $refund->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="btn btn-success btn-sm shadow-sm"
                                                    onclick="return confirm('Bạn có chắc chắn muốn xác nhận đã xử lý yêu cầu hoàn tiền này?')"
                            ...
                            <!-- Modal for QR Code -->
                            <div class="modal fade" id="qrModal{{ $refund->id }}" tabindex="-1" aria-labelledby="qrModalLabel{{ $refund->id }}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="qrModalLabel{{ $refund->id }}">Thông tin hoàn tiền</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body text-center">
                                            <img src="https://qr.sepay.vn/img?acc={{ $refund->bank_info['account_number'] ?? '31214717' }}&bank={{ $refund->bank_info['bank_name'] ?? 'ACB' }}&amount={{ $refund->checkout->final_refunded_amount ?? 0 }}&des=Hoan tien phong {{ urlencode($refund->checkout->contract->room->name ?? '') }}&template=compact" alt="QR Code" class="img-fluid mb-3" style="max-width: 200px;">
                                            <div class="text-start">
                                                <p><strong>Ngân hàng:</strong> {{ $refund->bank_info['bank_name'] ?? 'ACB' }}</p>
                                                <p><strong>Chủ tài khoản:</strong> {{ $refund->bank_info['account_holder'] ?? 'PHAN QUOC DUONG' }}</p>
                                                <p><strong>Số tài khoản:</strong> {{ $refund->bank_info['account_number'] ?? '31214717' }}</p>
                                            </div>
                                        </div>
