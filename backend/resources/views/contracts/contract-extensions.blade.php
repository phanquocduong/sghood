    ...
    <div class="card shadow-lg border-0 rounded-4">
        <div class="card-header bg-gradient text-white d-flex justify-content-between align-items-center rounded-top-4" style="background: linear-gradient(90deg, #007bff, #00c6ff);">
            <div class="d-flex align-items-center">
                <a href="{{ route('contracts.index') }}" class="btn btn-light btn-sm me-3 shadow-sm" style="transition: all 0.3s;" title="Quay lại danh sách hợp đồng">
                    ...
                    <tbody>
                        @forelse ($contractExtensions as $extension)
                            <tr class="table-row">
                                ...
                                <td>
                                    <a href="#" class="content-popup text-primary text-decoration-underline" data-bs-toggle="modal" data-bs-target="#contentModal{{ $extension->id }}">
                                        Xem chi tiết
                                    </a>
                                </td>
                                ...
                                <td>
                                    @if($extension->status == 'Chờ duyệt')
                                        <form action="{{ route('contracts.contract_extensions.update_status', $extension->id) }}" method="POST" class="status-form">
                                            @csrf
                                            <select class="form-select form-select-sm status-select" name="status" onchange="handleStatusChange(this, {{ $extension->id }})">
                                    ...
                                    @else
                                        ...
                                        @endphp
                                        <span class="badge bg-{{ $badgeClass }} py-2 px-3">{{ $statusText }}</span>
                                    @endif
                                </td>
                            </tr>

                            <!-- Content Modal -->
                            <div class="modal fade" id="contentModal{{ $extension->id }}" tabindex="-1" aria-labelledby="contentModalLabel{{ $extension->id }}" aria-hidden="true">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header bg-primary text-white">
                                            <h5 class="modal-title" id="contentModalLabel{{ $extension->id }}">Chi tiết nội dung gia hạn</h5>
                            ...
                            <div class="modal fade" id="rejectionModal{{ $extension->id }}" tabindex="-1" aria-labelledby="rejectionModalLabel{{ $extension->id }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header bg-danger text-white">
                                            <h5 class="modal-title text-white" id="rejectionModalLabel{{ $extension->id }}">Lý do từ chối</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <form action="{{ route('contracts.contract_extensions.update_status', $extension->id) }}" method="POST">
                                            @csrf
                                            <div class="modal-body">
                                                <input type="hidden" name="status" value="Từ chối">
                                                <div class="mb-3">
                                                    <label for="rejection_reason_{{ $extension->id }}" class="form-label">Hãy nhập lý do từ chối</label>
                                                    <textarea class="form-control" id="rejection_reason_{{ $extension->id }}" name="rejection_reason" rows="4" required></textarea>
                                                </div>
                                            </div>
                                            <div class="modal-footer">
