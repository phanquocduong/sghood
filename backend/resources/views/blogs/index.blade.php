    ...
    <div class="container-fluid py-5 px-4">
        <div class="card shadow-lg border-0" style="border-radius: 15px; background: #fff;">
            <div class="card-header bg-gradient text-white d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center"
                ...
                <!-- Tiêu đề luôn nằm trên -->
                <h6 class="mb-2 mb-sm-0 fw-bold w-100 text-start text-sm-start">
                    {{ __('Danh sách bài viết') }}
                </h6>

                <!-- Nhóm nút -->
                <div class="d-flex flex-row w-100 w-sm-auto justify-content-between justify-content-sm-end">
                    <a href="{{ route('blogs.create') }}" class="btn btn-primary me-2 shadow-sm w-50 w-sm-auto"
                        style="transition: all 0.3s;">
                        <i class="fas fa-plus me-1"></i> {{ __('Thêm bài viết') }}
                    </a>
                    <a href="{{ route('blogs.trash') }}" class="btn btn-danger shadow-sm w-50 w-sm-auto"
                        style="transition: all 0.3s;">
                            ...
                            @if ($blogs->count() > 0)
                                @foreach ($blogs as $blog)
                                    <tr class="table-row">
                                        ...
                                        <td><a
                                                href="{{ route('comments.index', $blog->id) }}">{{ $blog->comments_count }}</a>
                                        </td>
                                            ...
                                            <form action="{{ route('blogs.updateCategory', $blog->id) }}" method="POST">
                                                ...
                                                <select name="category" class="form-select form-select-sm"
                                                    ...
                                                    <option value="Kinh nghiệm"
                                                        {{ $blog->category == 'Kinh nghiệm' ? 'selected' : '' }}>Kinh
                                                        nghiệm
                                                    </option>
                                                </select>
                                            </form>
