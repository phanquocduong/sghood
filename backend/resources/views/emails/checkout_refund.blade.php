...
<head>
    ...
    <title>Thông báo hoàn tiền cọc</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .refund-header { background: #f8f9fa; border-radius: 10px; padding: 25px; margin: 25px 0; border-left: 4px solid #28a745; text-align: center; }
        .refund-header h2 { color: #28a745; margin-bottom: 5px; font-size: 20px; font-weight: bold; }
        .refund-header .status { color: #495057; font-size: 16px; margin-bottom: 15px; }
        ...
        .refund-table { width: 100%; border-collapse: collapse; margin: 25px 0; box-shadow: 0 2px 8px rgba(0,0,0,0.1); border-radius: 8px; overflow: hidden; }
        .refund-table th { background: #28a745; color: white; padding: 15px 12px; text-align: center; font-weight: 600; font-size: 14px; }
        .refund-table td { padding: 12px; text-align: center; border-bottom: 1px solid #dee2e6; }
        .refund-table tr:nth-child(even) { background-color: #f8f9fa; }
        .refund-table tr:hover { background-color: #e8f5e8; }
        .refund-table .item-name { text-align: left; font-weight: 500; }
        .refund-table .details { text-align: left; font-size: 13px; color: #666; }
        .refund-table .amount { text-align: right; font-weight: 600; }
        ...
        @media (max-width: 600px) {
            ...
            .refund-table th, .refund-table td { padding: 8px 6px; }
...
<div class="email-container">
    <div class="header">
        <h1>✅ Thông báo hoàn tiền cọc</h1>
        <p>Thông tin hoàn tiền cọc của bạn đã được xử lý</p>
    </div>
    ...
    <div class="content">
        ...
        <div class="success-message">
            <h4>🎉 Thông báo!</h4>
            <p>Quá trình hoàn tiền cọc của bạn đã được <strong style="color: #28a745;">hoàn tất</strong>. Vui lòng kiểm tra thông tin chi tiết dưới đây.</p>
        </div>

        <div class="refund-header">
            <h2>THÔNG BÁO HOÀN TIỀN CỌC</h2>
            <div class="status">Ngày hoàn tiền: {{ \Carbon\Carbon::parse($checkOutDate)->format('d/m/Y') }}</div>
        </div>
        ...
        <div class="customer-info">
            ...
            <div class="customer-right">
                ...
                <div class="info-item">
                    ...
                    <span class="info-value" style="color: #28a745; font-weight: bold;">Đã hoàn tiền</span>
                </div>
            </div>
        </div>
        ...
        <table class="refund-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="details">Phí khấu trừ từ tiền cọc</td>
                    <td class="amount" style="color: #dc3545;">-{{ number_format($checkout->deduction_amount, 0, ',', '.') }}đ</td>
                </tr>
                @endif
        ...
        <div class="payment-info">
            ...
            <div class="payment-details">
                ...
                <span class="payment-value">{{ number_format($checkout->deduction_amount, 0, ',', '.') }}đ</span>
            </div>
            @endif
            @if($checkout->final_refunded_amount)
            <div class="payment-details" style="border-top: 1px solid #ffeaa7; padding-top: 8px; margin-top: 8px;">
                <span class="payment-label"><strong>Tổng tiền hoàn trả:</strong></span>
                <span class="payment-value"><strong>{{ number_format($checkout->final_refunded_amount, 0, ',', '.') }}đ</strong></span>
            </div>
            @endif
        </div>

        <p class="message">
            Vui lòng truy cập trang quản lý hoàn tiền để xem chi tiết và xác nhận thông tin. Nhấp vào nút bên dưới để tiếp tục:
        </p>

        <div class="cta-section">
            <a href="https://sghood.com.vn/quan-ly/hoan-tien" class="cta-button" style="color: #ffffff;">
                <span class="icon">🏠</span> Xem chi tiết hoàn tiền
            </a>
        </div>
