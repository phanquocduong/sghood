                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-header border-0" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <form action="{{ route('notes.index') }}" method="GET">
                                <button type="submit" class="btn btn-link text-decoration-none p-0 small text-white">Xem tất cả</button>
                            </form>
                        </div>
                    </div>
                    <div class="card-body p-0">
                        <ul class="list-group list-group-flush">
                            ...
                            @else
                                <li class="list-group-item border-0 text-center text-muted py-4">
                                    <i class="fas fa-exclamation-triangle me-2"></i>Chưa tải được ghi chú: {{ session('error') ?? 'Lỗi không xác định' }}
                                </li>
                            @endisset
                        </ul>
                    </div>
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-header bg-white border-0 py-3">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('repair_requests.index') }}" class="text-decoration-none small text-dark">Xem tất cả</a>
                        </div>
                    </div>
                    <div class="card-body p-0">
                                        ...
                                        @forelse($repairRequests as $request)
                                            <tr>
                                                <td class="border-0 py-3 text-dark">
                                                    <span class="fw-semibold text-primary">{{ $request->contract->room->name ?? 'N/A' }}</span>
                                                </td>
                                                <td class="border-0 py-3 text-dark">{{ $request->contract->user->name ?? 'N/A' }}</td>
                                                <td class="border-0 py-3 text-dark">
                                                    <small class="text-muted">{{ $request->created_at ? $request->created_at->format('d/m/Y') : 'N/A' }}</small>
                                                </td>
                                                <td class="border-0 py-3">
                                                    <small class="text-muted">{{ Str::limit($request->description ?? $request->title ?? 'Không có mô tả', 30) }}</small>
                                                </td>
                                                <td class="border-0 py-3">
                                                ...
                                                <td class="border-0 py-3">
                                                    <a class="btn btn-sm btn-outline-primary" href="{{ route('repair_requests.show', $request->id) }}">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="6" class="border-0 text-center text-muted py-4">
                                                    <i class="fas fa-info-circle me-2"></i>Không có yêu cầu sửa chữa nào cần xử lý.
                                                </td>
                                            </tr>
                                        @endforelse
                                    @else
                                        <tr>
                                            <td colspan="6" class="border-0 text-center text-muted py-4">
                                                <i class="fas fa-exclamation-triangle me-2"></i>Không thể tải dữ liệu yêu cầu sửa chữa.
                                            </td>
                                        </tr>
                                    @endisset
                                </tbody>
                                ...
                                <div class="d-flex align-items-center">
                                    <div class="flex-shrink-0 me-3">
                                        <div class="bg-danger bg-opacity-10 rounded-circle d-flex align-items-center justify-content-center"
                                             style="width: 32px; height: 32px;">
                                            <i class="fas fa-message text-danger text-white" style="font-size: 12px;"></i>
                                        </div>
                                    </div>
                                    <div>
                                        <div class="fw-semibold mb-0 text-dark" style="font-size: 14px;">Hoàng Thị E</div>
                                        <small class="text-muted">Xem được rồi? <span class="text-primary">2 phút trước</span></small>
                                    </div>
                                </div>
                            </li>
                            <li class="list-group-item border-0 py-2">
                                <div class="d-flex align-items-ce   nter">
                                    ...
                                    <div>
                                        <div class="fw-semibold mb-0 text-dark" style="font-size: 14px;">Đỗ Văn F</div>
                                        <small class="text-muted">Trọ dơ quá! <span class="text-primary">2 phút trước</span></small>
                                    </div>
                                </div>
                            </li>
                        </ul>
